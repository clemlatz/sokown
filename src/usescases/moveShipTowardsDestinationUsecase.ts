...
export default async function moveShipTowardsDestinationUsecase(ship: Ship, logger: loggerFunction): Promise<Ship> {
  ...
  logger(`Ship ${ship.name} moved to ${newPosition} (going to ${ship.destination.name})`);
